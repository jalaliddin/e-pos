...
<div class="flex flex-wrap items-center gap-2 mb-6">
    <span class="text-sm font-medium text-gray-500 dark:text-gray-300 mr-1">Davr:</span>
    @foreach($periods as $key => $label)
    <button wire:click="setPeriod('{{ $key }}')"
        class="px-4 py-1.5 text-sm rounded-full border font-medium transition-all duration-150
            {{ $period === $key
                ? 'bg-primary-600 text-white border-primary-600 shadow-sm'
                : 'bg-white text-gray-600 border-gray-200 hover:border-primary-400 hover:text-primary-600 dark:bg-gray-700 dark:text-gray-200 dark:border-gray-500 dark:hover:border-primary-400 dark:hover:text-primary-300' }}">
        {{ $label }}
    </button>
    @endforeach
</div>
...
<div class="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow">
        <div class="flex justify-between items-start mb-3">
            <div class="p-2.5 rounded-xl bg-blue-50 dark:bg-blue-900/40">
            ...
            <span class="flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-full
                {{ $summary['revChange'] >= 0 ? 'bg-green-50 text-green-600 dark:bg-green-900/40 dark:text-green-300' : 'bg-red-50 text-red-600 dark:bg-red-900/40 dark:text-red-300' }}">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-300 mt-1">Jami savdo — {{ $periodLabel }}</p>
    </div>

    {{-- Profit --}}
    <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow">
        <div class="flex justify-between items-start mb-3">
            <div class="p-2.5 rounded-xl bg-green-50 dark:bg-green-900/40">
                <x-heroicon-o-arrow-trending-up class="w-5 h-5 text-green-600 dark:text-green-400"/>
            </div>
            <span class="text-xs font-semibold px-2 py-1 rounded-full bg-green-50 text-green-600 dark:bg-green-900/40 dark:text-green-300">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-300 mt-1">Sof foyda — {{ $periodLabel }}</p>
    </div>

    {{-- Orders --}}
    <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow">
        <div class="flex justify-between items-start mb-3">
            <div class="p-2.5 rounded-xl bg-purple-50 dark:bg-purple-900/40">
                <x-heroicon-o-shopping-bag class="w-5 h-5 text-purple-600 dark:text-purple-400"/>
            </div>
            @if(!is_null($summary['ordChange']))
            <span class="flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-full
                {{ $summary['ordChange'] >= 0 ? 'bg-green-50 text-green-600 dark:bg-green-900/40 dark:text-green-300' : 'bg-red-50 text-red-600 dark:bg-red-900/40 dark:text-red-300' }}">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-300 mt-1">Buyurtmalar — {{ $periodLabel }}</p>
    </div>

    {{-- Avg order --}}
    <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow">
        <div class="flex justify-between items-start mb-3">
            <div class="p-2.5 rounded-xl bg-orange-50 dark:bg-orange-900/40">
                <x-heroicon-o-cube class="w-5 h-5 text-orange-600 dark:text-orange-400"/>
            </div>
            <span class="text-xs font-medium text-gray-500 dark:text-gray-300">{{ number_format($summary['items']) }} dona</span>
        </div>
        <p class="text-2xl font-bold text-gray-900 dark:text-white leading-tight">{{ $fmt($summary['avgOrder']) }}</p>
        <p class="text-sm text-gray-500 dark:text-gray-300 mt-1">O'rtacha buyurtma — {{ $periodLabel }}</p>
    </div>
    ...
    <div class="lg:col-span-3 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-gray-400 dark:text-gray-400 mt-0.5">Kunlik daromad va foyda</p>
            </div>
            <x-heroicon-o-chart-bar class="w-5 h-5 text-gray-400 dark:text-gray-500"/>
        </div>
        <div class="p-5">
            ...
            <div class="flex items-end gap-0.5 h-32 overflow-x-auto pb-1" style="min-width:0">
                ...
                <div class="flex flex-col items-center flex-1 min-w-[18px] group relative">
                    <div class="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-gray-900 dark:bg-gray-950 text-white text-xs rounded-lg px-2.5 py-1.5 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 shadow-lg border border-gray-700">
                        <div class="font-semibold text-gray-100">{{ \Carbon\Carbon::parse($day->date)->format('d M') }}</div>
                    ...
                    <div class="relative w-full flex flex-col justify-end" style="height: {{ $maxH }}px">
                        <div class="w-full bg-blue-200 dark:bg-blue-800/60 rounded-t transition-all duration-500"
                            style="height: {{ $revH }}px">
                            <div class="w-full bg-green-400 dark:bg-green-500 rounded-t opacity-90"
                                style="height: {{ $proH }}px"></div>
                        </div>
                    </div>
                    <span class="text-[8px] text-gray-400 dark:text-gray-400 mt-1 whitespace-nowrap">
            ...
            <div class="flex items-center gap-4 mt-3">
                <span class="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
                    <span class="w-3 h-3 rounded-sm bg-blue-200 dark:bg-blue-800/60 inline-block"></span>Savdo
                </span>
                <span class="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
                    <span class="w-3 h-3 rounded-sm bg-green-400 dark:bg-green-500 inline-block"></span>Foyda
                </span>
            </div>
            @else
            <div class="flex flex-col items-center justify-center h-32 text-gray-400 dark:text-gray-500">
                ...
                <p class="text-sm text-gray-400 dark:text-gray-400">Ma'lumot yo'q</p>
            </div>
            @endif
        </div>
    </div>
    ...
    <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-gray-400 dark:text-gray-400 mt-0.5">Foyda bo'yicha</p>
            </div>
        ...
        <div class="p-5 space-y-4">
            ...
            <div>
                <div class="flex items-center justify-between mb-1.5">
                    <span class="text-sm font-medium text-gray-700 dark:text-gray-200 truncate max-w-[130px]">
                        {{ $cat->category_name ?: 'Kategoriyasiz' }}
                    </span>
                    <div class="flex items-center gap-2 shrink-0">
                        <span class="text-xs text-gray-400 dark:text-gray-400">{{ number_format($cat->total_qty) }} dona</span>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-gray-400 dark:text-gray-400 mt-0.5">Qaysi soatlarda savdo ko'p</p>
            </div>
        ...
        <div class="p-5">
            ...
            <div class="grid grid-cols-12 gap-1">
                ...
                @php
                    ...
                    $opacity = $cnt > 0 ? max(0.2, $intens) : 0.07;
                @endphp
                <div class="group relative">
                    ...
                    <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 bg-gray-900 dark:bg-gray-950 text-white text-xs rounded px-2 py-1 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 border border-gray-700">
                        {{ str_pad($h, 2, '0', STR_PAD_LEFT) }}:00 — {{ $cnt }} ta buyurtma
                    </div>
                </div>
                @endfor
            </div>
            <div class="flex justify-between mt-2 text-[10px] text-gray-400 dark:text-gray-400">
                @for($h = 0; $h < 24; $h += 3)
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-gray-400 dark:text-gray-400 mt-0.5">5 ta va undan kam qolganlar</p>
            </div>
        ...
        <div class="divide-y divide-gray-50 dark:divide-gray-700">
            ...
            <div class="flex items-center justify-between px-6 py-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                <div class="flex items-center gap-3">
                    <img src="{{ $product->getImageUrl() }}" class="w-8 h-8 rounded-lg object-cover bg-gray-100 dark:bg-gray-700">
                    <div>
                        <p class="text-sm font-medium text-gray-800 dark:text-gray-100">{{ $product->name }}</p>
                        <p class="text-xs text-gray-400 dark:text-gray-400">{{ $product->category->name ?? '—' }}</p>
                    </div>
                </div>
            ...
            <div class="flex flex-col items-center justify-center py-8">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">Barcha mahsulotlar yetarli</p>
            </div>
            @endforelse
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-gray-400 dark:text-gray-400 mt-0.5">Miqdor bo'yicha top 10</p>
            </div>
            ...
            <div class="flex items-center gap-4 px-6 py-3.5 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors border-b border-gray-50 dark:border-gray-700/50 last:border-0">
                <span class="text-base w-6 text-center shrink-0">
                    @if($i < 3)
                        {{ $medals[$i] }}
                    @else
                        <span class="text-sm text-gray-400 dark:text-gray-500">{{ $i+1 }}</span>
                    @endif
                </span>
                ...
                <div class="text-right shrink-0">
                    ...
                    <p class="text-xs text-gray-400 dark:text-gray-400">{{ $fmt($p->total_revenue) }}</p>
                </div>
            </div>
            @empty
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-gray-400 dark:text-gray-400 mt-0.5">Foyda bo'yicha top 10</p>
            </div>
        ...
        <div class="overflow-hidden">
            ...
            <div class="flex items-center gap-4 px-6 py-3.5 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors border-b border-gray-50 dark:border-gray-700/50 last:border-0">
                ...
                <div class="text-right shrink-0">
                    ...
                    <p class="text-xs text-gray-400 dark:text-gray-400">{{ number_format($p->total_qty) }} dona</p>
                </div>
            </div>
            @empty
            <div class="flex flex-col items-center justify-center py-10 text-gray-400 dark:text-gray-500">
