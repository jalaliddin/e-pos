...
<div class="mx-auto">
        ...
        <button wire:click="selectCategory(null)"
            class="px-2 py-1 text-xs rounded-full border transition-colors
                {{ $selectedCategory === null
                    ...
                    : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-100 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600' }}">
            Hammasi
        </button>
    ...
    <div class="grid grid-cols-2 md:grid-cols-1 lg:grid-cols-3 gap-2">
        ...
        <div wire:click="addToCart({{ $product->id }})"
            class="relative bg-white border border-gray-300 rounded overflow-hidden cursor-pointer hover:border-primary-400 transition-colors dark:bg-gray-800 dark:border-gray-600">
            ...
            <p class="text-gray-600 p-2 text-sm dark:text-gray-300">{{ $product->name }} ({{ $product->quantity }})</p>
            <p class="text-gray-600 p-2 pt-0 text-md dark:text-gray-300">{{ $currency_symbol . $product->price }}</p>
        ...
        @empty
        <div class="col-span-3 text-center text-gray-400 py-8 text-sm">Mahsulot topilmadi</div>
        @endforelse
    </div>
    ...
    <div class="mt-3">
        {{ $products->links('livewire::pagination.simple') }}
    </div>
    @endif
</div>
